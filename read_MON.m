function [nit,tt,ke,et,tm,sm,um,vm,wm,cfl,zm,sct,SD] = read_MON(rac_nam,list_on,nrec)
% [nit,tt,ke,et,tm,sm,um,vm,wm,cfl,zm,sct,SD] = read_MON(rac_nam,list_on,[nrec])
%-----------------------------------------------------------------
...
% output "SD": =1 if 4th stats is Standard-Deviation (Monitor sufix "_sd") ; =0 if "_del2"
%-----------------------------------------------------------------
...
SD=1; %- set to 1 to return Standard-Deviation (4th stats); = 0 to return "_del2"
ke=zeros(nrec,5);
et=zeros(nrec,4);
tm=zeros(nrec,4);
sm=zeros(nrec,4);
...
if list_on(2) == 1, [et,msg] =read_MON_1v([rac_nam,'.et'],nit,nrec,SD) ;
                if length(msg) ~= 1, msgA=sprintf([msgA,msg]) ; end ; end
if list_on(3) == 1, [tm,msg] =read_MON_1v([rac_nam,'.t_'],nit,nrec,SD) ;
                if length(msg) ~= 1, msgA=sprintf([msgA,msg]) ; end ; end
if list_on(4) == 1, [sm,msg] =read_MON_1v([rac_nam,'.s_'],nit,nrec,SD) ;
                if length(msg) ~= 1, msgA=sprintf([msgA,msg]) ; end ; end
if list_on(5) == 1, [um,msg] =read_MON_1v([rac_nam,'.u_'],nit,nrec,SD) ;
                if length(msg) ~= 1, msgA=sprintf([msgA,msg]) ; end ; end
if list_on(6) == 1, [vm,msg] =read_MON_1v([rac_nam,'.v_'],nit,nrec,SD) ;
                if length(msg) ~= 1, msgA=sprintf([msgA,msg]) ; end ; end
if list_on(7) == 1, [wm,msg] =read_MON_1v([rac_nam,'.w_'],nit,nrec,SD) ;
                if length(msg) ~= 1, msgA=sprintf([msgA,msg]) ; end ; end
if list_on(8) == 1, [cfl,msg]=read_MON_1v([rac_nam,'.cf'],-nit,nrec);
                if length(msg) ~= 1, msgA=sprintf([msgA,msg]) ; end ; end
%--------
if list_on(9) == 1, [zm,msg]=read_MON_1v([rac_nam,'.z_'],nit,nrec,SD);
...
function [var,msg] = read_MON_1v(nam,nit,nrec,SD)
prt=0; msg=' ';
ll=size(nam,2);
fprintf([' ',nam(ll-2:ll)]);
if (nargin < 4), SD=1; end
if SD == 1, sufx='NXAS'; else sufx='NXAD'; end
% fprintf('%s',sufx);
if nit < 0,
